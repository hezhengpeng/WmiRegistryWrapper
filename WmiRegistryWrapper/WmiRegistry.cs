...
namespace WmiRegistryWrapper
{
    ...
    public class WmiRegistry
    {
        ...
        public static string GetOrCreateString(WmiRegistry registry, WmiRegistryHive hive, string subKey, string valueName, string defaultValue = "")
        {
            // 1. 是否存在 Key，不存在则创建
            registry.TryCreateSubKey(hive, subKey);

            // 2. 获取值
            var value = registry.GetValue(hive, subKey, valueName, RegistryValueType.String);

            // 3. 如果值不存在，则写入默认值
            if (value == null)
            {
                registry.TrySetValue(hive, subKey, valueName, defaultValue);
                return defaultValue;
            }
        ...
        private ConnectionOptions Options { get; } =
            new()
            {
        ...
        /// <summary>
        ///  Deletes a SubKey in the specified tree.(被删除的项不能含有子项)
        /// </summary>
        /// <param name="registryHive">A registry tree, also known as a hive, that contains the SubKey path.</param>
        /// <param name="subKeyPath">The key to be deleted.</param>
        /// <returns>True if the operation was successful, False if not.</returns>
        public bool TryDeleteSubKey(WmiRegistryHive registryHive, string subKeyPath)
        {
            return (uint)
                    ExecuteCommand(registryHive, subKeyPath, RegCommand.DeleteKey)
        ...
        /// <returns></returns>
        public bool TryDeleteSubTree(WmiRegistryHive hive, string subKeyPath)
        {
            // 递归删除子键
            var subKeys = (IEnumerable<string>?)
                ExecuteCommand(hive, subKeyPath, RegCommand.EnumKey).GetPropertyValue("sNames");
            //string[] subKeys = subKeysResult["sNames"] as string[];

            if (subKeys != null && subKeys.Count() > 0)
            {
                foreach (string subKey in subKeys)
                {
                    string childPath = subKeyPath + "\\" + subKey;
                    TryDeleteSubTree(hive, childPath);
                }
            }

            // 删除当前键
            return (uint)
                    ExecuteCommand(hive, subKeyPath, RegCommand.DeleteKey)
                        .GetPropertyValue("ReturnValue") == 0;
        }
        ...
        {
            var methodParams = Registry.GetMethodParameters(nameof(RegCommand.DeleteValue));

            methodParams["hDefKey"] = registryHive;
            methodParams["sSubKeyName"] = subKeyPath;
            methodParams["sValueName"] = valueName;

            var result = Registry.InvokeMethod(
                "DeleteValue",
                methodParams,
                new InvokeMethodOptions()
            );

            var vv = (uint)result.GetPropertyValue("ReturnValue");

            return result != null && vv == 0;
        }
        ...
        /// <returns>True if the operation was successful, False if not.</returns>
        public bool TryCreateSubKey(WmiRegistryHive registryHive, string subKeyPath)
        {
            return (uint)
                    ExecuteCommand(registryHive, subKeyPath, RegCommand.CreateKey)
                        .GetPropertyValue("ReturnValue") == 0;
        }
        ...
        public IEnumerable<string>? EnumerateSubKeys(
            WmiRegistryHive registryHive,
            string subKeyPath
        )
        {
            return (IEnumerable<string>?)
                ExecuteCommand(registryHive, subKeyPath, RegCommand.EnumKey)
                    .GetPropertyValue("sNames");
        }

        public IEnumerable<RegEntity> EnumerateValueNames(
            WmiRegistryHive registryHive,
            string subKeyPath
        )
        {
        ...
        )
        {
            ...
            {
                RegistryValueType.DWord => GetValueCommand(
                        registryHive,
                        subKeyPath,
                        valueName,
                ...
                RegistryValueType.QWord => GetValueCommand(
                        registryHive,
                        subKeyPath,
                        valueName,
                    ...
                    .GetPropertyValue("uValue"),
                RegistryValueType.Binary => GetValueCommand(
                        registryHive,
                        subKeyPath,
                        valueName,
                    ...
                    .GetPropertyValue("uValue"),
                RegistryValueType.String => GetValueCommand(
                        registryHive,
                        subKeyPath,
                        valueName,
                ...
                RegistryValueType.ExpandedString => GetValueCommand(
                        registryHive,
                        subKeyPath,
                        valueName,
                    ...
                    .GetPropertyValue("sValue"),
                RegistryValueType.MultiString => GetValueCommand(
                        registryHive,
                        subKeyPath,
                        valueName,
        ...
        private ManagementBaseObject ExecuteCommand(
            WmiRegistryHive registryHive,
            string subKeyPath,
        ...
        private ManagementBaseObject SetValueCommand(
            WmiRegistryHive registryHive,
            string subKeyPath,
            string valueName,
            object value,
        ...
        private static bool IsExpandedString(string str) =>
